...
$$
is a Boolean polynomial of degree at most $2$ in the $25r$ binary coordinates of $X$. Adding a linear word keeps degree at most $2<25r$. The parity of the weight is the total sum of that Boolean polynomial over $\mathbb F_2^{25r}$; every monomial omits at least one variable, so the sum is even. Thus every word in $\mathcal C_\tau$ also lies in $X_n$.
...
$$
which lies between $0$ and $2046$. Therefore $R(z)$ modulo $2048$ determines all ten marker bits uniquely, and after subtracting them the remaining term determines $\operatorname{wt}(z)$. Hence every isometry fixing the zero word preserves Hamming weight and each of the ten marked families.
...
\sigma_i(1)=1.
$$
For $i\neq j$, the line $E(E_i+E_j)$ maps to $E(f_i+f_j)$. Hence
$$
...
Now $s(E_i+tE_j)$ lies on the line $E(E_i+tE_j)$, so
$$
...
$$
Therefore $\sigma$ is a field automorphism of $E$. Hence
$$
\sigma(t)=t^{2^k}
$$
for a unique
...
0\leq k<r.
$$
There is a unique $E$-linear bijection $T:V\to V$ such that
$$
\phi(X)=T(X^{2^k}),
$$
...
Preservation of $\mathcal R$ says that $T$ preserves rank-one matrices in both directions. The maximal $5$-dimensional $E$-subspaces all of whose nonzero elements have rank $1$ are exactly
$$
U_{[u]}=\{uv^T:v\in E^5\}
$$
and
...
W_{[v]}=\{uv^T:u\in E^5\}.
$$
Indeed, if $uv^T$ and $xy^T$ are independent and both $u,x$ and $v,y$ are independent, then after changing row and column bases their sum contains a $2\times2$ identity block and has rank at least $2$. Thus a rank-at-most-one subspace must have a common column direction or a common row direction.

The marker $\mathcal U$ says that the $U$-ruling is preserved rather than exchanged. Hence the $W$-ruling is preserved as well.
...
(E_i+E_{i'})(E_j+E_{j'})^T
$$
must map to rank one. In the row and column bases determined by $a_i,a_{i'}$ and $b_j,b_{j'}$, this gives
$$
\lambda_{ij}\lambda_{i'j'}
=\lambda_{ij'}\lambda_{i'j}.
$$
Therefore the scalar matrix $(\lambda_{ij})$ has multiplicative rank one. Explicitly,
$$
\lambda_{ij}=r_i c_j
$$
with
...
\qquad(Y\in M_5(E)).
$$
The two nonzero $E$-linear functionals have the same kernel, so they are proportional. The matrix trace pairing is nondegenerate, hence there is a unique
$$
\gamma\in E^\times
$$
such that
...
and the equation
$$
\operatorname{tr}(C\phi^{-1}(Y))=0
$$
may be raised to the $2^k$-th power. The scalar $\gamma^{-1}$ then disappears from the kernel condition, giving (2).

Preservation of
$$
\mathcal E=\{h_{E_{00}},\ldots,h_{E_{44}}\}
$$
therefore implies that $A E_{ii}A^{-1}$ is proportional to some $E_{jj}$. Both matrices have trace $1$, so the proportionality factor is $1$. Thus $A$ permutes the five coordinate lines, and hence
$$
A=D_0P
$$
...
$$
The five cyclic successive gaps of $C_5$ are
$$
1,2,4,8,16,
$$
all distinct. Hence no nonzero translation of $\mathbb Z/31\mathbb Z$ preserves $C_5$: a translation preserving the set would cyclically shift its ordered gap sequence, and the distinct gaps force the zero shift.

The marker $h_D$ is fixed. Since $D_0$ commutes with diagonal matrices, (2) gives
$$
P D^{2^k}P^{-1}=\lambda D
$$
for some $\lambda\in E^\times$. Ratios of diagonal entries show that $\lambda\in\mathbb F_{2^5}^\times=\langle\eta\rangle$, say $\lambda=\eta^h$. On exponents modulo $31$, the equality says
$$
2^k C_5=C_5+h.
$$
Multiplication by $2^k$ already permutes $C_5$, so the preceding no-translation observation gives
$$
h=0.
$$
The five diagonal entries of $D$ are distinct, so the permutation is uniquely determined. If
$$
s\equiv k\pmod5,
\qquad 0\leq s<5,
$$
...
$$
in characteristic $2$, and conjugation and Frobenius preserve this trace. Hence
$$
\lambda=1. \tag{4}
$$

Comparing the coefficient of $E_{j,j+1}$ in (4) gives
$$
\frac{d_j}{d_{j+1}}
\eta^{\,2^k b_{j-s}}
=
...
$$
the five equations are consistent. Scalar multiplication of $A$ has no effect on conjugation, so normalize
$$
d_0=1.
$$
Then (5) determines all $d_j$ uniquely. Since $2^k\equiv2^s\pmod{31}$, the answer depends only on $s=k\bmod5$.
...
A_s=D_sP_s. \tag{7}
$$
Direct substitution in (5) verifies every row of the table. Thus the projective matrix $A$ is already forced to be $A_s$ once $k\bmod5$ is known.
...
=P_sR^{2^k}P_s^{-1}. \tag{8}
$$
For (8) to define one of the five hyperplanes in $\mathcal B$, it must be proportional to some
$$
P_tRP_t^{-1}.
$$
At at least three diagonal positions both matrices have entry $1$, so the proportionality scalar must be $1$. Therefore
$$
\rho^{2^k}=\rho.
$$
Since $\rho$ has order $7$ and the multiplicative order of $2$ modulo $7$ is $3$, this is equivalent to
$$
3\mid k. \tag{9}
$$
Conversely, if $3\mid k$, then $R^{2^k}=R$ and (8) merely cyclically permutes the five members of $\mathcal B$. Hence (9) is the exact restriction.

Thus the allowed Frobenius exponents are
$$
k\in\{0,3,6,\ldots,r-3\}. \tag{10}
$$
There are $r/3$ of them.
...
$$
If $s\neq t$, nondegeneracy of the absolute trace pairing gives some $x$ with
$$
\operatorname{Tr}((s+t)x)=1,
$$
so the polar forms of $\mathbf c_s$ and $\mathbf c_t$ differ.

The coefficient $c_2$ is invariant under conjugation because it is a characteristic-polynomial coefficient, and it satisfies
$$
c_2(\gamma X)=\gamma^2c_2(X),
\qquad
c_2(X^{2^k})=c_2(X)^{2^k}. \tag{11}
$$
Using (1), (11), and invariance of the absolute trace under Frobenius gives
$$
P_\phi(\mathcal C_s)
=\mathcal C_{\,s^{2^k}\gamma^{-2}}. \tag{12}
$$
Preservation of the marked family $\mathcal C_\tau$ therefore gives
$$
\tau^{2^k}\gamma^{-2}=\tau,
$$
or
$$
\gamma^2=\tau^{2^k-1}. \tag{13}
$$
The squaring map on $E^\times$ is bijective, so there is exactly one solution,
$$
\gamma_k
=\left(\tau^{2^k-1}\right)^{2^{r-1}}. \tag{14}
$$

Step 12: State the normal form, verify sufficiency, and count
...
s(k)\in\{0,1,2,3,4\}
$$
be the residue of $k$ modulo $5$, let $A_{s(k)}$ be given by (6)-(7), and define
$$
\Phi_k:M_5(E)\to M_5(E),
\qquad
\Phi_k(X)
=\gamma_k A_{s(k)}X^{2^k}A_{s(k)}^{-1}. \tag{15}
$$

Every zero-fixing isometry is exactly the coordinate permutation induced by one $\Phi_k$. Conversely, each $\Phi_k$ preserves $E$-lines, rank, the chosen ruling, and the trace-zero condition. The matrices $A_{s(k)}$ preserve $\mathcal E$, equations (3) and (6) ensure preservation of $h_D$ and $h_K$, condition $3\mid k$ preserves $\mathcal B$, and (13) preserves $\mathcal C_\tau$. Hence every displayed map preserves all marker bits and Hamming weight, so it preserves $d_n$.

Restoring translations, every isometry is uniquely
$$
(F_{a,k}(x))_X
=a_X+x_{\Phi_k^{-1}(X)}, \tag{16}
$$
where
$$
a\in X_n,
...
|X_n|=2^{n-1}
$$
and there are $r/3$ admissible exponents,
$$
\boxed{
...
- rank-one Segre geometry and ruling reconstruction
- trace-zero rigidity
- diagonal idempotent frames
- Singer-cycle exponent rigidity modulo $31$
- weighted cyclic conjugacy equations
