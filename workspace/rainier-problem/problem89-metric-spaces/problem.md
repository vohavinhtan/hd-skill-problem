...
$$\mathcal L=\{\ell_C:C\ne0\},\quad \mathcal P=\{\chi_L:\dim_E L=1\},\quad \mathcal R=\{\chi_{EX}:X\ne0,\ \operatorname{rank}X=1\},$$
$$\mathcal U=\{\chi_{\{uv^T:v\in E^5\}}:0\ne u\in E^5\},\quad \mathcal Z=\{\chi_{EX}:X\ne0,\ \operatorname{tr}X=0\}.$$
For $C\ne0$ let $H_C=\{X:\operatorname{tr}(CX)=0\}$ and $h_C=\chi_{H_C}$. Let $E_{ij}$ be the matrix units (indices mod $5$), and let $P_se_i=e_{i+s}$. Set
...
Let $c_2(X)$ be the coefficient of $T^3$ in $\det(TI_5+X)$, $(\mathbf c_s)_X=\operatorname{Tr}(s\,c_2(X))$, and $\mathcal C_\tau=\{\mathbf c_\tau+\ell_C:C\in V\}$, where $\ell_0=0$. For $x\ne y$, set $z=x+y$ and
...
where $\mathbf1_S$ is the membership indicator; set $d_n(x,x)=0$. Determine $|\operatorname{Iso}(X_n,d_n)|$ exactly.
