...
\qquad u\geq u_0.
$$
This tends to infinity because the integral grows like $u^5/5$ while the denominator grows like $u^4$. It also gives uniform stability. If $u_0<0$, then $\sin\theta<0$ until $u=0$, so $\dot p=-2\sin\theta\,p+2>0$ on that interval. For $u_0\geq0$, either $u_0\geq u/2$, which gives $p(u)\geq p_0/16$, or $0\leq u_0<u/2$. In the latter case, $u<1$ gives $p(u)\geq p_0/4$, while $u\geq1$ gives
$$
p(u)\geq\frac{p_0}{4u^4}+\frac{31u}{320}
\geq\frac{31}{320}p_0^{1/5}.
$$
The last inequality follows by separating $u\leq p_0^{1/5}$ from $u\geq p_0^{1/5}$. So $\inf p(u)\to\infty$ as $p_0\to\infty$. On the invariant ray $\theta=\pi$, the equation is $\dot x=-2x^2$. This proves that $a=0$ is included when $\lambda=1$.
