...
Rank $3$ is impossible. Then $\ker\Psi=\langle A_0\rangle$ for an invertible matrix $A_0$. Precompose with the cone automorphism $X\mapsto A_0X$: the new map
$$
\widetilde\Psi(X)=\Psi(A_0X)
$$
has
$$
\ker\widetilde\Psi=A_0^{-1}\ker\Psi=\langle I\rangle.
$$
Thus it is enough to treat $\ker\Psi=\langle I\rangle$. For rank-one $X$,
$$
...
$$
Left and right multiplication by invertible matrices preserves matrix rank and sends $D$ to $A^{-T}DB^{-T}$, so row and column reduction shows that there are only two cases: $\operatorname{rank}D=1$, represented by $E_{11}$, and $\operatorname{rank}D=2$, represented by $I$. If $D=E_{11}$, then $H_D$ is $x_{11}=0$ and
$$
\det\begin{pmatrix}0&b\\c&d\end{pmatrix}=bc.
$$
It is invertible exactly when $b=c=1$, with two choices of $d$, so among the seven nonzero matrices exactly five are singular. If $D=I$, then $H_D$ is $\operatorname{tr}X=0$; writing
$$
X=\begin{pmatrix}a&b\\c&a\end{pmatrix}
$$
gives $\det X=a+bc$. For $a=0$, the singular cases $bc=0$ give three matrices, one of which is zero; for $a=1$, singularity requires $b=c=1$, giving one more. Hence exactly three nonzero matrices are singular. Thus no hyperplane contains six rank-one points, contradiction. Hence rank $3$ does not occur.
...
\{0,I,C,I+C\}
$$
with both $C$ and $I+C$ invertible. For $C=R$ or $R^2$ we have $I+R=R^2$ and $I+R^2=R$, so both work. For the other three nonidentity invertibles,
$$
I+J=\begin{pmatrix}1&1\\1&1\end{pmatrix},\quad
I+JR=\begin{pmatrix}0&1\\0&0\end{pmatrix},\quad
I+JR^2=\begin{pmatrix}0&0\\1&0\end{pmatrix},
$$
all singular. Hence the normalized external line is uniquely
$$
E_0=\{0,I,R,R^2\}.
$$
...
$$
with $A,B\in\operatorname{GL}_2(\mathbb F_2)$. In the first case $\Psi^{-1}(Y)=A^{-1}YB^{-1}$. In the second case
$$
\Psi^{-1}(Y)=(A^{-1}YB^{-1})^T,
$$
and trace is unchanged by transpose. Hence in either case
$$
...
$$
Replacing $Q$ by a suitable composition of $Q$ with such a right multiplication makes the transformed map equal to $\Psi_0$.

It remains to verify that postcomposition by $Q$ preserves the count, including the phase condition. Every cone automorphism has one of the two forms in Step 3. Fix a rank-one $X$ and write $C=\Psi_1(X)$. If
$$
Q(C)=ACB,
$$
set
...
$$
Conversely, this formula plainly puts $T(X)$ in $\langle\ell(X)^T\rangle$ for every rank-one $X$, so it is also sufficient.
...
7,5,3,1.
$$
Since the four lengths are distinct, the number of such permutations is
$$
...
For each $u$, cycle lengths and cycle sign products are invariants of conjugacy in the signed permutation action. The defining condition depends only on these data (with the four cycles distinguished by their distinct lengths), so the corresponding subset of $G=\operatorname{Gal}(L/\mathbb Q)$ is stable under conjugation. From the stated description of $G$, choosing $\sigma\in S_{16}$ and the sixteen functions $h_j\in\mathcal H_n$ determines a unique group element, hence
$$
|G|=16!\,|\mathcal H_n|^{16}.
$$
